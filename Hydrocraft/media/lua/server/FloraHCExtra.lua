...
function HCFloraGather(items, result, player)
	...
	if not player:isOutside() then player:Say(getText("ContextMenu_There_are_no_flora_indoors")); --Проверка помещения
		return;
	end
	...
	if luck <= 8 then player:Say(getText("ContextMenu_I_found_nothing"));
		return;
	end
	...
	-- getting seeds from gathering flora in autum [получение семян собирательством осенью при навыке больше 7]
	if skill >7 and (getGameTime():getMonth() + 1) > 8 and (getGameTime():getMonth() + 1) < 11 and ZombRand(30) == 1 then
		luck = ZombRand(11);
		if luck == 0 then player:getInventory():AddItem("Hydrocraft.HCCottonseeds"); --Семена хлопка
		elseif luck == 1 then player:getInventory():AddItem("Hydrocraft.HCHempseeds"); --Семена конопли
		elseif luck == 2 then player:getInventory():AddItem("Hydrocraft.HCPoppyseeds"); --Семена мака
		elseif luck == 3 then player:getInventory():AddItem("Hydrocraft.HCRubbertreeseeds"); --Семена каучукового дерева
		elseif luck == 4 then player:getInventory():AddItem("Hydrocraft.HCSpirulinaseeds"); --Семена спирулины
		elseif luck == 5 then player:getInventory():AddItem("Hydrocraft.HCBasilseeds"); --Семена базилика
		elseif luck == 6 then player:getInventory():AddItem("Hydrocraft.HCOreganoseeds"); --Семена орегано
		elseif luck == 7 then player:getInventory():AddItem("Hydrocraft.HCBananaseeds"); --Семена банана
		elseif luck == 8 then player:getInventory():AddItem("Hydrocraft.HCOnionseeds"); --Семена лука
		elseif luck == 9 then player:getInventory():AddItem("Hydrocraft.HCWheat"); --Семена пшеницы
		elseif luck == 10 then player:getInventory():AddItem("Hydrocraft.HCSoyBeans"); --Семена соевых бобов
		end 
	...
	player:getXp():AddXP(Perks.PlantScavenging, 1); --Получение опыта за сбор
end
