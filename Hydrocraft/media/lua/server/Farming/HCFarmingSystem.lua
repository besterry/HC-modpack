...
-- Безопасная версия getLuaObjectByIndex для обработки null-объектов
function SFarmingSystem:getLuaObjectByIndex(index)
	local globalObject = self.system:getObjectByIndex(index-1)
	if globalObject then
		return globalObject:getModData()
	else
		return nil
	end
end
...
function SFarmingSystem:changeHealth() -- функция для изменения здоровья растения
	...
	local countRemoved = 0 --Количество удаленных растений
	...
	self:removerBadPlants() -- удаление всех плохих растений
end -- function
...
local oneClean = false
function SFarmingSystem:removerBadPlants() -- удаление всех плохих растений один раз после запуска сервера
	if oneClean then return end
	local countRemoved = 0
    for i=1,self:getLuaObjectCount() do
        local luaObject = self:getLuaObjectByIndex(i)
        if luaObject then
			if luaObject.state == "plow" or luaObject.state == "dry" or luaObject.state == "rotten" or luaObject.state == "destroy" then
	...
	oneClean = true
	print("WorldCleaner: removed " .. countRemoved .. " plants")
end