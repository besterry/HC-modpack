...
function PM_ISMenu:initialise()
    ...
    --Баланс игрока
    self.balance = ISLabel:new(x + self.balancetext:getWidth() + 5, y, FONT_HGT_MEDIUM, "", 255, 215, 0, 1, UIFont.Medium, true)
    self.balance:initialise()
    self.balance:instantiate()
    self:addChild(self.balance)

    --бонусный баланс
    self.bonus = ISLabel:new(x + self.balance:getWidth() + 145, y, FONT_HGT_MEDIUM, "", 0, 0.75, 1, 1, UIFont.Medium, true)
    self.bonus:initialise()
    self.bonus:instantiate()
    self:addChild(self.bonus)

    --кнопка убежище
    self.SHBTN = ISButton:new(x, self.balancetext.y + 25, btnWid, btnHgt, getText("IGUI_SafeHouseButton"), self, PM_ISMenu.onClick);
    ...
    --кнопка Мой магазин
    self.SHOPBTN = ISButton:new(x, self.SHBTN.y + 35, btnWid, btnHgt, getText("IGUI_PMShopButton"), self, PM_ISMenu.onClick);
    ...
    --кнопка Магазин
    self.Shop = ISButton:new(x, self.SHOPBTN.y + 35, btnWid, btnHgt, getText("IGUI_PMShop"), self, PM_ISMenu.onClick);
    ...
    self.cancel = ISButton:new(self:getWidth() - btnWid - 10, self:getHeight() - padBottom - btnHgt, btnWid, btnHgt, getText("UI_Close"), self, PM_ISMenu.onClick);
...
function PM_ISMenu:onClick(button)
    if button.internal == "CANCEL" then
        ...
        if PM_SafeHouseMenu.instance then PM_SafeHouseMenu.instance:close()  PM_SafeHouseMenu.instance = nil end 
        if PM_ShopMenu.instance then PM_ShopMenu.instance:close() PM_ShopMenu.instance = nil end          
