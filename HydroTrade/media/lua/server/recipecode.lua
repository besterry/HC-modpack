...
function Recipe.GetItemTypes.DullKnife(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("DullKnife"))
	addExistingItemType(scriptItems, "BreadKnife")
	addExistingItemType(scriptItems, "ButterKnife")
end
...
function Recipe.GetItemTypes.Scissors(scriptItems)
	...
	addExistingItemType(scriptItems, "Scissors")
end
...
function Recipe.GetItemTypes.Glue(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Glue"))
	addExistingItemType(scriptItems, "Glue")
	addExistingItemType(scriptItems, "WoodGlue")
end

function Recipe.GetItemTypes.Fork(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Fork"))
	addExistingItemType(scriptItems, "Fork")
end

function Recipe.GetItemTypes.Corkscrew(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Corkscrew"))
	addExistingItemType(scriptItems, "Corkscrew")
end

function Recipe.GetItemTypes.Disinfectant(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Disinfectant"))
	addExistingItemType(scriptItems, "Disinfectant")
end

function Recipe.GetItemTypes.Egg(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Egg"))
	addExistingItemType(scriptItems, "Egg")
	addExistingItemType(scriptItems, "WildEggs")
end

function Recipe.GetItemTypes.GasMask(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("GasMask"))
	addExistingItemType(scriptItems, "GasMask")
end

function Recipe.GetItemTypes.HazmatSuit(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("HazmatSuit"))
	addExistingItemType(scriptItems, "HazmatSuit")
end

function Recipe.GetItemTypes.Liquor(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Liquor"))
	addExistingItemType(scriptItems, "WhiskeyFull")
end

function Recipe.GetItemTypes.Milk(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Milk"))
	addExistingItemType(scriptItems, "Milk")
end

function Recipe.GetItemTypes.MortarPestle(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("MortarPestle"))
	addExistingItemType(scriptItems, "MortarPestle")
end

function Recipe.GetItemTypes.Petrol(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Petrol"))
	addExistingItemType(scriptItems, "PetrolCan")
end

function Recipe.GetItemTypes.Razor(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Razor"))
	addExistingItemType(scriptItems, "Razor")
end

function Recipe.GetItemTypes.Spoon(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Spoon"))
	addExistingItemType(scriptItems, "Spoon")
end
...
function Recipe.GetItemTypes.Tape(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Tape"))
	addExistingItemType(scriptItems, "Scotchtape")
	addExistingItemType(scriptItems, "Ducttape")
end
...
function Recipe.GetItemTypes.BakingFat(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("BakingFat"))
	addExistingItemType(scriptItems, "Butter")
	addExistingItemType(scriptItems, "Lard")
	addExistingItemType(scriptItems, "OilOlive")
	addExistingItemType(scriptItems, "OilVegetable")
end

function Recipe.GetItemTypes.Cheese(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Cheese"))
	addExistingItemType(scriptItems, "Cheese")
	addExistingItemType(scriptItems, "Processedcheese")
end

function Recipe.GetItemTypes.Flour(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Flour"))
	addExistingItemType(scriptItems, "Flour")
	addExistingItemType(scriptItems, "Cornflour")
end

function Recipe.GetItemTypes.Chocolate(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Chocolate"))
	addExistingItemType(scriptItems, "Chocolate")
	addExistingItemType(scriptItems, "ChocolateChips")
end
...
function Recipe.GetItemTypes.SewingNeedle(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("SewingNeedle"))
	addExistingItemType(scriptItems, "Needle")
end

function Recipe.GetItemTypes.Oil(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("Oil"))
	addExistingItemType(scriptItems, "OilOlive")
	addExistingItemType(scriptItems, "OilVegetable")
end

function Recipe.GetItemTypes.PizzaSauce(scriptItems)
	scriptItems:addAll(getScriptManager():getItemsTag("PizzaSauce"))
	addExistingItemType(scriptItems, "TomatoPaste")
end
...
function Recipe.OnTest.IsWorn(item)
	if instanceof(item, "Clothing") then
		return item:isWorn()
	end
	return true		
...
-- checks to make whole containers of milk are used to make the mildew spray cure
function Recipe.OnTest.WholeMilk(item)
	if not item:hasTag("Milk") then return true end
    ...
    if item:isRotten() then return not ((baseHunger * 0.45) > hungerChange) end
    return not ((baseHunger * 0.75) > hungerChange)
end

-- checks to make sure items using the egg tag in recipes are whole and not partially-eaten eggs as appropriate
function Recipe.OnTest.WholeEgg(item)
	if item:isCooked() then return false end
	if not item:hasTag("Egg") then return true end
	local baseHunger = math.abs(item:getBaseHunger())
	local hungerChange = math.abs(item:getHungerChange())
    if item:isFresh() then return not (baseHunger > hungerChange) end
    return not ((baseHunger * 0.75) > hungerChange)
...
function Recipe.OnTest.FullPetrolBottle(item)
	if not item:hasTag("Petrol") then return true; end
    return item:getUsedDelta() == 1; -- only return true if the bottle is full
end
...
function Recipe.OnCreate.MakeBowlOfSoup4(items, result, player)
    ...
    for i=0,items:size() - 1 do
        local item = items:get(i)
        if item:getType() == "PotOfSoup" or item:getType() == "PotOfSoupRecipe" or item:getType() == "RicePan" or item:getType() == "PastaPan" or item:getType() == "PastaPot"or item:getType() == "RicePot" or item:getType() == "WaterPotRice" or item:getType() == "WaterPotPasta" or item:getType() == "WaterSaucepanRice" or item:getType() == "WaterSaucepanPasta" then
...
function Recipe.OnCreate.SliceBreadDough(items, result, player)
    for i=0,items:size() - 1 do
        local item = items:get(i)
        if item:getType() == "BreadDough" then			
...
function Recipe.OnCreate.SlicePie(items, result, player)
    for i=0,items:size() - 1 do
        local item = items:get(i)
        if item:getType() == "PieWholeRaw" or item:getType() == "PieWholeRawSweet" or item:getType() == "CakeRaw" then
...
function Recipe.GetItemTypes.CraftLogStack(scriptItems)
    scriptItems:addAll(getScriptManager():getItemsTag("SheetRope"));
end
...
function Recipe.OnCreate.DismantleTVRemote(items, result, player)
    local success = 60;
    player:getInventory():AddItem("Base.ElectronicsScrap");
...
function Recipe.OnCreate.SliceSalami(items, result, player)
    for i=0,items:size() - 1 do
        local item = items:get(i)
...
function Recipe.OnCreate.GetBiscuit(items, result, player, selectedItem)
...
function Recipe.OnCreate.GetCookies(items, result, player, selectedItem)
	if selectedItem:isBurnt() then result:setBurnt(true) end
...
function Recipe.OnCanPerform.SlicePizza(recipe, playerObj, item)
	if item and item:getType() == "PizzaWhole" then return true end;
    return item and (item:isCooked() or item:isBurnt());
end

function Recipe.OnCreate.SlicePizza(items, result, player, selectedItem)
	if selectedItem:isBurnt() then result:setBurnt(true) end
	if selectedItem:isCooked() then result:setCooked(true) end
    result:setBaseHunger(selectedItem:getBaseHunger() / 6);
    result:setHungChange(selectedItem:getHungChange() / 6);
    -- result:setUnhappyChange(selectedItem:getUnhappyChange() / 6);
    -- result:setBoredomChange(selectedItem:getBoredomChange() / 6);
    result:setUnhappyChange(selectedItem:getUnhappyChange() );
    result:setBoredomChange(selectedItem:getBoredomChange() );
    result:setCalories(selectedItem:getCalories() / 6);
    result:setProteins(selectedItem:getProteins() / 6);
    result:setLipids(selectedItem:getLipids() / 6);
    result:setCarbohydrates(selectedItem:getCarbohydrates() / 6);
	if selectedItem:getDisplayName() then
		result:setName(selectedItem:getDisplayName() .. " " .. getText("Tooltip_food_Slice"));
		result:setCustomName(true);
	end
...
function Recipe.OnCreate.BeanBowl(items, result, player)
    for i=0,items:size() - 1 do
        local item = items:get(i);
...
function Recipe.GetItemTypes.DismantleCamera(scriptItems)
    scriptItems:addAll(getScriptManager():getItemsTag("Camera"));
end

function Recipe.OnCreate.MakeOatmeal(items, result, player)
    result:setHeat(2.5);
	result:setCooked(true);
end
