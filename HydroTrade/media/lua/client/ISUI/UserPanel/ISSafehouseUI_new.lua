...
function ISSafehouseUI:initialise()
    ...
    --IGUI_SafehouseUI_RemoveConfirm
...
function ISSafehouseUI:onClickRefreshTime()
    ...
    local timeLeftText = getText("IGUI_TimeLeft") .. " " .. timeLeftDays ..  getText("IGUI_daysLeft") .. timeLeftHours ..  getText("IGUI_hoursLeft") .. timeLeftMinutes ..  getText("IGUI_minLeft")
...
function ISSafehouseUI:onClick(button)
    ...
    if button.internal == "REFRESHTIME" then
        self:onClickRefreshTime();
    end;
    if button.internal == "RESETTIME" then
