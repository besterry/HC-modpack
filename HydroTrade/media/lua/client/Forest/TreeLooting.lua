--HCFern листья папоротника
--HCFiddleheadfern рахисы папоротника
...
local treeLoot = { --Всех деревьев 8 типов (0-3 лето, 4-7 зима) 0 - молодое, 3 - старое
        ...
        ["e_carolinasilverbell_1_0"] = {{  item = "Hydrocraft.HCBeechnut", chance = 20 }}, --HCBeechnut - Буковый орех
        ["e_carolinasilverbell_1_1"] = {{  item = "Hydrocraft.HCBeechnut", chance = 30 }},
        ["e_carolinasilverbell_1_2"] = {{  item = "Hydrocraft.HCBeechnut", chance = 40 }},
        ["e_carolinasilverbell_1_3"] = {{  item = "Hydrocraft.HCBeechnut", chance = 50 }},
        ["e_carolinasilverbell_1_4"] = {{  item = "Hydrocraft.HCBeechnut", chance = 10 }},
        ["e_carolinasilverbell_1_5"] = {{  item = "Hydrocraft.HCBeechnut", chance = 20 }},
        ["e_carolinasilverbell_1_6"] = {{  item = "Hydrocraft.HCBeechnut", chance = 30 }},
        ["e_carolinasilverbell_1_7"] = {{  item = "Hydrocraft.HCBeechnut", chance = 40 }},

        --Боя́рышник пету́шья шпо́ра, или Боярышник шпо́рцевый, — дерево, вид рода Боярышник семейства Розовые.
        ["e_cockspurhawthorn_1_0"] = {{ item = "Hydrocraft.HCChestnut", chance = 20 }}, --HCChestnut --Каштан
        ["e_cockspurhawthorn_1_1"] = {{ item = "Hydrocraft.HCChestnut", chance = 30 }},
        ["e_cockspurhawthorn_1_2"] = {{ item = "Hydrocraft.HCChestnut", chance = 40 }},
        ["e_cockspurhawthorn_1_3"] = {{ item = "Hydrocraft.HCChestnut", chance = 50 }},
        ["e_cockspurhawthorn_1_4"] = {{ item = "Hydrocraft.HCChestnut", chance = 10 }},
        ["e_cockspurhawthorn_1_5"] = {{ item = "Hydrocraft.HCChestnut", chance = 20 }},
        ["e_cockspurhawthorn_1_6"] = {{ item = "Hydrocraft.HCChestnut", chance = 30 }},
        ["e_cockspurhawthorn_1_7"] = {{ item = "Hydrocraft.HCChestnut", chance = 40 }},
        ...
        ["e_easternredbud_1_0"] = {{ item = "Base.Cherry", chance = 30}}, --Base.Cherry
        ["e_easternredbud_1_1"] = {{ item = "Base.Cherry", chance = 40}},
        ["e_easternredbud_1_2"] = {{ item = "Base.Cherry", chance = 50}},
        ["e_easternredbud_1_3"] = {{ item = "Base.Cherry", chance = 60}},
        ["e_easternredbud_1_4"] = {{ item = "Base.Cherry", chance = 20}},
        ["e_easternredbud_1_5"] = {{ item = "Base.Cherry", chance = 25}},
        ["e_easternredbud_1_6"] = {{ item = "Base.Cherry", chance = 30}},
        ["e_easternredbud_1_7"] = {{ item = "Base.Cherry", chance = 35}},
        ...
        ["e_yellowwood_1_0"] = {{ item = "Hydrocraft.HCAcorn", chance = 20},{ item = "Hydrocraft.HCOakleaves", chance = 20}}, --HCAcorn - желудь
        ["e_yellowwood_1_1"] = {{ item = "Hydrocraft.HCAcorn", chance = 30},{ item = "Hydrocraft.HCOakleaves", chance = 30}}, --HCOakleaves листья дуба
        ["e_yellowwood_1_2"] = {{ item = "Hydrocraft.HCAcorn", chance = 40},{ item = "Hydrocraft.HCOakleaves", chance = 40}},
        ["e_yellowwood_1_3"] = {{ item = "Hydrocraft.HCAcorn", chance = 50},{ item = "Hydrocraft.HCOakleaves", chance = 50}},
        ["e_yellowwood_1_4"] = {{ item = "Hydrocraft.HCAcorn", chance = 10},{ item = "Hydrocraft.HCOakleaves", chance = 10}},
        ["e_yellowwood_1_5"] = {{ item = "Hydrocraft.HCAcorn", chance = 20},{ item = "Hydrocraft.HCOakleaves", chance = 20}},
        ["e_yellowwood_1_6"] = {{ item = "Hydrocraft.HCAcorn", chance = 30},{ item = "Hydrocraft.HCOakleaves", chance = 30}},
        ["e_yellowwood_1_7"] = {{ item = "Hydrocraft.HCAcorn", chance = 40},{ item = "Hydrocraft.HCOakleaves", chance = 40}},
}
...
-- Создаём действие для выполнения анимации и лутания
local function CreateLootAction(obj, player)
...
function LootingTree(obj, player)
    ...
    for _, loot in ipairs(lootTable) do
        if ZombRand(0, 100) < loot.chance then
            player:getInventory():AddItem(loot.item)
            count = count + 1
            --player:Say("I found " .. loot.item:getFullType() .. "!")
        end
    end
    ...
    for _, loot in ipairs(defaultLoot) do
        if ZombRand(0, 100) < loot.chance then
            ...
            count = count + 1
            --local itemName = item:getName() -- Получаем имя предмета
            --player:Say(getText("IGUI_Found_TreeLooting") .. itemName .. "!")
        end
    end
    player:getXp():AddXP(Perks.PlantScavenging, 0.1*count)
    -- Обновляем информацию о лутании
    local interval = ZombRand(1, 3) --Респ 2-11 дней
    obj:getModData().TimeRespawn = interval
    obj:getModData().TimeLoot = getGameTime():getWorldAgeHours() --getWorldAgeDays
...
function OnTreeClick(playerNum, context, worldObjects)
    ...
    for i, obj in ipairs(worldObjects) do
        if instanceof(obj, "IsoTree") and not alreadyHandled then      
            ...
            alreadyHandled = true -- Устраняем дублирование 150+5 160
            ...
            if lastLootTime + respawnTime > getGameTime():getWorldAgeHours() then --Если дерево уже луталось за LootTime часов
                ...
                context:addOption(getText("IGUI_Tree_Looting"), obj, CreateLootAction, player)
            end
        end
    end
end
