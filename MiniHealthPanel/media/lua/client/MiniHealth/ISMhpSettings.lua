...
function ISMhpSettings:populateOptions()
	self.setFunction = {}
	self:addOption(getText("IGUI_Always_visible"), mhpHandle.alwaysShow, function(mhpHandle, selected)
		mhpHandle:toggleAlwaysShow(selected)
	end)
	self:addOption(getText("IGUI_Health_bar"), mhpHandle.showHpBar, function(mhpHandle, selected)
		mhpHandle:toggleHpBar(selected)
	end)
	self:addOption(getText("IGUI_Lock_window"), not mhpHandle.moveWithMouse, function(mhpHandle, selected)
