...
--- Фольга ---
--- Фольга для поджигания батерейкой
function recipe_Make_Foil_Lighter(items, sourceItem, result, player)
	local player = getSpecificPlayer(0);
	local DeltaBattery_Foil;
	for i=0, items:size()-1 do
		if items:get(i):getType() == "Battery" then
			DeltaBattery_Foil = items:get(i):getUsedDelta();
			break
		end
	end

	local DATAPlayer = player:getModData();
	DATAPlayer.DeltaBattery_FoilLigther = DeltaBattery_Foil;
end
...
-------------------
-------------------
--- Сделать крышку из фольги ---
--- Фольга отличается... Сигаретная фольга/Алюминиевая фольга
function recipe_MakeCapFoil_OnCreate(sourceItem, result, player)
	local player = getPlayer();
	local foil_type;
	for i=0, sourceItem:size()-1 do
		if sourceItem:get(i):getType() == "SMFoil" or sourceItem:get(i):getType() == "ChocolateFoil" or sourceItem:get(i):getType() == "Aluminum" then
			foil_type = tostring(sourceItem:get(i):getType());
			break
		end
	end
...
function recipe_MakeHolesFoil_OnCreate(sourceItem, result, player)
	local foil_type;
	for i=0, sourceItem:size()-1 do
		if sourceItem:get(i):getType() == "SMCrumpledWithFoilCap" then
			local bulbulyator = sourceItem:get(i);
			local DATABulbulyator = bulbulyator:getModData();	
			foil_type = DATABulbulyator.FoilType;
			break
		end
	end
