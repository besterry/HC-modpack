...
//nightvision (HydroNV tuned):
float contrast2 = 0.55;
const vec3 lumvec = vec3(0.30, 0.59, 0.11);
float intensityAdjust = 1.15;
...
// HydroNV: static grain without rotating timer
vec4 rnm_static(in vec2 tc)
{
...
float pnoise3D_static(in vec3 p)
{
	vec3 pi = permTexUnit * floor(p) + permTexUnitHalf;
	vec3 pf = fract(p);

	float perm00 = rnm_static(pi.xy).a;
	vec3 grad000 = rnm_static(vec2(perm00, pi.z)).rgb * 4.0 - 1.0;
	float n000 = dot(grad000, pf);
	vec3 grad001 = rnm_static(vec2(perm00, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
	float n001 = dot(grad001, pf - vec3(0.0, 0.0, 1.0));

	float perm01 = rnm_static(pi.xy + vec2(0.0, permTexUnit)).a;
	vec3 grad010 = rnm_static(vec2(perm01, pi.z)).rgb * 4.0 - 1.0;
	float n010 = dot(grad010, pf - vec3(0.0, 1.0, 0.0));
	vec3 grad011 = rnm_static(vec2(perm01, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
	float n011 = dot(grad011, pf - vec3(0.0, 1.0, 1.0));

	float perm10 = rnm_static(pi.xy + vec2(permTexUnit, 0.0)).a;
	vec3 grad100 = rnm_static(vec2(perm10, pi.z)).rgb * 4.0 - 1.0;
	float n100 = dot(grad100, pf - vec3(1.0, 0.0, 0.0));
	vec3 grad101 = rnm_static(vec2(perm10, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
	float n101 = dot(grad101, pf - vec3(1.0, 0.0, 1.0));

	float perm11 = rnm_static(pi.xy + vec2(permTexUnit, permTexUnit)).a;
	vec3 grad110 = rnm_static(vec2(perm11, pi.z)).rgb * 4.0 - 1.0;
	float n110 = dot(grad110, pf - vec3(1.0, 1.0, 0.0));
	vec3 grad111 = rnm_static(vec2(perm11, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
	float n111 = dot(grad111, pf - vec3(1.0, 1.0, 1.0));
...
vec3 screenNightvision(in vec3 pixel, in vec3 noise, in vec2 uv) {
	vec3 Diffuse = pixel;
	float intensity = (0.299 * Diffuse.r) + (0.587 * Diffuse.g) + (0.114 * Diffuse.b);
	float nightMix = clamp(NightValue, 0.0, 1.0);

	Diffuse = mix(Diffuse, vec3(intensity), (nightMix * 0.75) * (1.0 - intensity));
	Diffuse = ((1.0 - nightMix) * Diffuse) + (nightMix * Diffuse * Diffuse) * 1.35;

	Diffuse.rg -= nightMix * 0.22 * (1.0 - intensity);
	Diffuse.b += nightMix * 0.22 * (1.0 - intensity);

	vec3 col = contrast(desaturate(Diffuse, 0.08), 1.15);
	float invintensity = 1.0 - intensity;
	invintensity = invintensity * invintensity;
...
void main()
{
	...
	if(NightVisionGoggles<0.5) {
		...
		vec3 rotOffset = vec3(1.425, 3.892, 5.835);
		vec2 rotCoordsR = coordRot(UV, timer * 0.10 + rotOffset.x);
		vec3 noise = vec3(pnoise3D(vec3(rotCoordsR * vec2(width / grainsize, height / grainsize), timer * 0.03)));
		col = screenNightvision(pixel, noise, UV);
	}
