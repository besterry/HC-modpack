...
local function applyPanic(panicAmo,stressAmo,playBreathing,forceAwake,Request)
    if Request:isLocalPlayer() and Request:getPlayerNum() == 0 then
        ...
        if playBreathing == true and ZombRand(2) == 1 then
...
local function insideSchizoAttack(Request)
    if Request:isLocalPlayer() and Request:getPlayerNum() == 0 then
        ...
        applyPanic(50,0.25,true,true,Request)
    end
end
...
function MakeNewPlayer(_,newplayer)
    if newplayer:isLocalPlayer() and newplayer:getPlayerNum() == 0 and player == nil then figureOutPlayer()
        if LoadedEvents == true then FigureOutBuilding() end
    end
end
...
function makePlayerSound(player)
...
function HourEvent()
    if player ~= nil then
        ...
        elseif randNum > (8*ChlorModifier) and player:HasTrait("schizophrenia") and player:isLocalPlayer() and player:getPlayerNum() == 0 then
            ...
            randNum = randNum - (randNum*player:getStats():getStress())*0.5
            if randNum <= (20*ChlorModifier) then makePlayerSound(player) end
            randNum = ZombRand(100)+1
...
function FakeAlarm(_player)
    if _player == player and _player:isLocalPlayer() and _player:getPlayerNum() == 0 then
...
function RollHitFakeBite(_,plrChar)
    if plrChar:getStats() == player:getStats() and player:HasTrait("schizophrenia") and plrChar:isLocalPlayer() and plrChar:getPlayerNum() == 0 then
        playerdata = player:getModData()
        local IsOnPills, _ = checkChlorpromazineHours()
        local ChlorModifier = 1
        if IsOnPills then ChlorModifier = 0.20 end
...
function ApplyDeathDepression(deadPlayer)
    if not deadPlayer:isLocalPlayer() then
...
function PillEffects()
    if player:isLocalPlayer() and player:getPlayerNum() == 0 then
        ...
        elseif playerdata.ChlorpromazineHours >= 42 then ApplyPillEffects(player,0,25,0.12,0.06,0.12,ZombRand(100)+1,ZombRand(100)+1) return
        elseif playerdata.ChlorpromazineHours >= 36 then ApplyPillEffects(player,0,25,0.06,0.04,0.04,100,100) return
        elseif playerdata.ChlorpromazineHours >= 30 then ApplyPillEffects(player,0,0,0.03,0,0.02,100,100) return end
    end
end
...
function HitHalZombieCheck(zombie)
    if zombie == halzombie and player:isLocalPlayer() and player:getPlayerNum() == 0 then
        halzombie:removeFromWorld()
        halzombie:removeFromSquare()
        halzombie = nil
        Events.OnTick.Remove(CanSeeCheck)
...
function CreateHallucination()
    if player:HasTrait("schizophrenia") and player:isLocalPlayer() and player:getPlayerNum() == 0 and not player:isAsleep() then
        if halzombie ~= nil then
            halzombie:removeFromWorld()
            halzombie:removeFromSquare()
            halzombie = nil
